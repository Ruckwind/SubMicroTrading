...
public final class RouterSharedData implements SMTComponent {
    ...
    private final ConcurrentPooledElementsMap<ZString,EventHandler>   _idToSrc;
    
    public RouterSharedData( String id, int expectedOrders ) {
        super();
        _id = id;
        
        _idToSrc = new ConcurrentPooledElementsMap<ZString,EventHandler>( expectedOrders );         
    }
     ...
     */
    public final void storeReturnRoute( final ZString clOrdId, final EventHandler replyHandler ) {
        _idToSrc.put( clOrdId, replyHandler );
    }

    public final EventHandler findHandler( ZString clOrdId, ZString orderId ) {
        
        EventHandler mh = _idToSrc.get( orderId );
    ...
    public final EventHandler findHandler( ZString clOrdId ) {
        
        EventHandler mh = _idToSrc.get( clOrdId );
     ...
     *       doesnt make copy here as dont know which thread invocation is on
     */
    public final EventHandler matchUpOrderIdToClOrdId( ZString clOrdId, ZString orderId ) {
        
        final EventHandler mh = _idToSrc.get( clOrdId );
