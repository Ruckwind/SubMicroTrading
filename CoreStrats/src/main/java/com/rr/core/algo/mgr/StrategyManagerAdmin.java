...
public class StrategyManagerAdmin implements StrategyManagerAdminMBean {
    ...
    public String fullPauseStrategy( String strategyId ) {
        ...
        try {
            strat.setCompRunState( CompRunState.HardPause );

            return "fullyPauseStrategy " + strategyId + " NOW " + strat.getCompRunState();
    ...
    public String semiPauseStrategy( String strategyId ) {
        ...
        try {
            strat.setCompRunState( CompRunState.PassivePause );

            return "semiPauseStrategy " + strategyId + " NOW " + strat.getCompRunState();
    ...
    public String resumeStrategy( String strategyId ) {
        ...
        try {
            strat.setCompRunState( CompRunState.Active );

            return "resumeStrategy " + strategyId + " NOW " + strat.getCompRunState();
    ...
    public String fullPauseAll() {
        ...
        for( Strategy<?> strat : strats ) {
            _log.info( "StrategyManagerAdmin.fullPauseAll ... pausing " + strat.id() );

            if ( strat.setCompRunState( CompRunState.HardPause ) ) {
                ++cnt;
            }
        }
    ...
    public String semiPauseAll() {
        ...
        for( Strategy<?> strat : strats ) {
            _log.info( "StrategyManagerAdmin.semiPauseAll ... pausing " + strat.id() );

            if ( strat.setCompRunState( CompRunState.PassivePause ) ) {
                ++cnt;
            }
        }
    ...
    public String resumeAll() {
        ...
        for( Strategy<?> strat : strats ) {
            _log.info( "StrategyManagerAdmin.resumeAll ... resuming " + strat.id() );

            if ( strat.setCompRunState( CompRunState.Active ) ) {
                ++cnt;
            }
        }
    ...
    private void list( AdminReply reply, Strategy<?> strat ) {
        if ( strat != null ) {
            ...
            reply.add( strat.getCompRunState().toString() );
