...
public class ExecTableModel extends AbstractTableModel implements EventHandler {
    ...
    static final class MiniExec implements Comparable<MiniExec> {
        ...
        MiniExec( CommonExecRpt exec ) {
            ...
            _price = Utils.zeroNull( exec.getPrice() );
            _orderQty = Utils.zeroNull( exec.getOrderQty() );
            _currency = exec.getCurrency();
            _side = exec.getSide();
            _avePx = Utils.zeroNull( exec.getAvgPx() );
            _cumQty = Utils.zeroNull( exec.getCumQty() );
