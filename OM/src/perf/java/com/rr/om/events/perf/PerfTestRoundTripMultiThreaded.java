...
//        doTestThreaded(   25000, 32,   0, ackDelay  );
    ...
    private long doPerfRun( String comment,
                            ...
                            int threads,
                            int total,
                            Class<? extends EventQueue> classQ,
                            int producerDelayMS,
        ...
        Set<ClientSessionEmulator> producers = new LinkedHashSet<>();
        ...
        for ( ClientSessionEmulator pr : producers ) {
            ...
            long[] stats = pr.getRoundTripTimes();
    ...
    private void doTestThreaded( int count, int threads, int prodDelayMS, int ackDelayMS ) throws InstantiationException, IllegalAccessException {
        ...
        if ( Utils.getMaxCores() > 3 ) {
            int total = count * threads;

            _log.info( "\n===========================================================================\n" );

            _log.info( "\nCONCURRENT LINKED QUEUE tot=" + total );
            long dougQ = perf( "SUN", count, threads, total, JavaConcEventQueue.class, ackDelayMS, prodDelayMS );

            _log.info( "\nNEW LINKED QUEUE tot=" + total );
            long concQ = perf( "RRCAS", count, threads, total, ConcLinkedEventQueueSingle.class, ackDelayMS, prodDelayMS );
    ...
    private long perf( String comment,
        ...
        EventQueue              exSessQ = classQ.newInstance();
        ...
        EventDispatcher dispatcher = new ThreadedDispatcher( "PROC1", classQ.newInstance(), ThreadPriority.Processor );
