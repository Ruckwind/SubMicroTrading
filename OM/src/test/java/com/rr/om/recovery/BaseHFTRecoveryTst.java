...
public abstract class BaseHFTRecoveryTst extends BaseRecoveryTst {
    ...
    protected FixDecoder _fullDecoder = FixTestUtils.getFullDecoder44();
    ...
    protected void checkExec( CommonExecRpt execToCheck,
        ...
        if ( expStatus.getIsTerminal() ) {
            assertEquals( 0, execToCheck.getLeavesQty(), OK_DELTA );
        } else {
            double leavesQty = baseRequest.getOrderQty() - expCumQty;

            assertEquals( leavesQty, execToCheck.getLeavesQty(), OK_DELTA );
        }

        assertEquals( baseRequest.getClOrdId(), execToCheck.getClOrdId() );
        assertEquals( expCumQty, execToCheck.getCumQty(), OK_DELTA );
        ...
        assertEquals( baseRequest.getOrderQty(), execToCheck.getOrderQty(), OK_DELTA );
    ...
    protected void checkTrade( TradeBase execToCheck,
        ...
        assertEquals( expLastQty, execToCheck.getLastQty(), OK_DELTA );
        assertEquals( expLastPx, execToCheck.getLastPx(), OK_DELTA );
    }

    protected void replay( DMARecoverySessionContext ctx, long key ) {
        Event msg = ctx.regenerate( key );
        if ( ctx.isInbound() ) {
    ...
    protected void setupACK( String clOrdId, int qty, double px, PKeys pk ) {
        ...
        NewOrderAck mack       = FixTestUtils.getMarketACK( _fullDecoder, clOrdId, qty, px, mktOrderId, new ViewString( "ACK" + clOrdId ) );
    ...
    protected void setupCancelled( String clOrdId, String origClOrdId, int qty, double px, PKeys pk ) {
        ...
        Cancelled mrep = FixTestUtils.getCancelled( _msgBuf, _fullDecoder, _clOrdId, _origClOrdId, qty, px, _orderId, _execId );
    ...
    protected void setupReplaced( String clOrdId, String origClOrdId, int newQty, double newPx, PKeys pk ) {
        ...
        Replaced mrep = FixTestUtils.getMarketReplaced( _msgBuf, _fullDecoder, _clOrdId, _origClOrdId, newQty, newPx, _orderId, _execId );
        pk._mInKey = persist( _exchange1, mrep, true );
        _proc.handle( mrep ); // MCXL now recycled, also CCAN recycled
    ...
    protected void setupTrade( int qty, double px, int cumQty, double avePx, String execId, PKeys keySRC, PKeys pkTrade, OrdStatus status ) {
        ...
        _execId.copy( execId );
        TradeNewImpl mtrade = (TradeNewImpl) FixTestUtils.getMarketTradeNew( buf, _fullDecoder, _mktOrderId, req.getClOrdId(),
                                                                             req, qty, px, _execId );
    ...
    protected void setupTradeCancel( String refExecId,
        ...
        TradeCancelImpl mtrade = (TradeCancelImpl)
                FixTestUtils.getMarketTradeCancel( buf, _fullDecoder, _mktOrderId, req.getClOrdId(),
    ...
    protected void setupTradeCorrect( String refExecId,
        ...
        TradeCorrectImpl mtrade = (TradeCorrectImpl)
                FixTestUtils.getMarketTradeCorrect( buf, _fullDecoder, _mktOrderId, req.getClOrdId(),
                                                    req, qty, px, _execId, _refExecId, status );
        mtrade.setOrdStatus( status );
        mtrade.setCumQty( cumQty );
        mtrade.setAvgPx( avePx );
