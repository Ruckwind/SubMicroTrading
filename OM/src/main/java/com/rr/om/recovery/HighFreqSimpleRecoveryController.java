...
public class HighFreqSimpleRecoveryController implements DMARecoveryController {
    ...
    private static final class RecOrder {
        ...
        private RecoverableSession _sess;
        ...
        public void dump( ReusableString buf ) {
            buf.append( "Order [" );
            if ( _sess != null ) buf.append( _sess.getComponentId() );
    ...
    @SuppressWarnings( "MismatchedQueryAndUpdateOfCollection" ) private final ConcurrentHashMap<ZString, ZString>   _downClOrdIdToUpClOrdId;
    ...
    private Event _upChainRoot;
    private Event _upChainTail;
    private Event _downChainRoot;
    private Event _downChainTail;
    ...
    public HighFreqSimpleRecoveryController( String id, int expOrders, int totalSessions, EventProcessorImpl proc ) {
        _id                         = id;
        _idToUpstreamOrder          = new ConcurrentHashMap<>( expOrders, 0.75f, totalSessions );
        _idToDownstreamOrder        = new ConcurrentHashMap<>( expOrders, 0.75f, totalSessions );
        _idToDownstreamOrderVersion = new ConcurrentHashMap<>( expOrders, 0.75f, totalSessions );
        _downClOrdIdToUpClOrdId     = new ConcurrentHashMap<>( expOrders, 0.75f, totalSessions );
        _upClOrdIdToDownClOrdId     = new ConcurrentHashMap<>( expOrders, 0.75f, totalSessions );
        _orderIdSymToVersion        = new ConcurrentHashMap<>( expOrders, 0.75f, totalSessions );
    ...
    public void start() {
        ...
        _startTimeMS = ClockFactory.get().currentTimeMillis();
    }
    ...
    protected Event getDownChain() {
        return _downChainRoot;
    }

    protected Event getUpstreamChain() {
        return _upChainRoot;
    }
    ...
    private void checkForPosResendUp( RecOrder upOrder, RecOrdVer upOrdVer, ReusableString buf ) {
        ...
        while( curExec != null ) {
            if ( (curExec._execFlags & RecoverableSession.PERSIST_FLAG_CONFIRM_SENT) == 0 ) {
                ...
                Event m = regenerateUpExec( upOrder, upOrdVer, buf, curExec );
    ...
    private Event dispatchEvents( Event msg ) {
        while( msg != null ) {
            Event next = msg.getNextQueueEntry();
            ...
            RecoverableSession hdl = (RecoverableSession) msg.getEventHandler();
    ...
    private void doProcessInbound( DMARecoverySessionContext ctx, long persistKey, Event msg, short persistFlags ) {
    ...
    private void doProcessOutbound( DMARecoverySessionContext ctx, long persistKey, Event msg, short persistFlags ) {
        final SessionDirection sd = ctx.getSession().getDirection();
    ...
    private void downstreamInHandleCancelReject( CancelReject msg, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
    ...
    private void downstreamInHandleCancelled( Cancelled msg, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
        downstreamInReqReply( msg.getOrigClOrdId(), msg, OrdStatus.Canceled, persistKey, ctx, persistFlags );
    }

    private void downstreamInHandleNewOrderAck( NewOrderAck msg, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
        downstreamInReqReply( null, msg, OrdStatus.New, persistKey, ctx, persistFlags );
    }

    private void downstreamInHandleOtherExec( CommonExecRpt msg, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
    ...
    private void downstreamInHandleRejected( Rejected msg, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
        downstreamInReqReply( msg.getOrigClOrdId(), msg, OrdStatus.Rejected, persistKey, ctx, persistFlags );
    }

    private void downstreamInHandleReplaced( Replaced msg, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
        downstreamInReqReply( msg.getOrigClOrdId(), msg, OrdStatus.Replaced, persistKey, ctx, persistFlags );
    }

    private void downstreamInHandleVagueReject( VagueOrderReject msg, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
     ...
     */
    private void downstreamInReqReply( ZString origClOrdId, CommonExecRpt msg, OrdStatus status, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
        RecOrder order;
    ...
    private void downstreamOutHandleCancelReplaceRequest( CancelReplaceRequest msg, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
        ...
        setLinkId( msg.getParentClOrdId(), persistKey, ctx, newVersion._clOrdIdChain, newVersion );
    }

    private void downstreamOutHandleCancelRequest( CancelRequest msg, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
        ZString   newClOrdId  = msg.getClOrdId();
        ZString   prevClOrdId = msg.getOrigClOrdId();
        RecOrder  order       = getOrder( _idToDownstreamOrder, prevClOrdId );
        RecOrdVer curVersion  = getVersion( order, prevClOrdId );
    ...
    private void downstreamOutHandleNewOrderSingle( NewOrderSingle msg, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
        ZString   clOrdId = msg.getClOrdId();
        ...
        setLinkId( msg.getParentClOrdId(), persistKey, ctx, version._clOrdIdChain, version );
    }
    ...
    private boolean forceCancelDownstream( RecOrder downOrder, RecOrdVer downOrdVer ) {
        RecoverableSession downSess = downOrder._sess;
    ...
    private void forceCancelUpstream( RecOrder upOrder, RecOrdVer upOrdVer ) {
        RecoverableSession upSess = upOrder._sess;
    ...
    private TradeBase handleMissingTradeCorrect( OrderRequest src,
        ...
        if ( mktTrade.getMultiLegReportingType() != MultiLegReportingType.LegOfSpread ) {
            ...
            double qtyDiff = origTrade.getLastQty() - mktTrade.getLastQty();
    ...
    private TradeBase handleMissingTradeNew( OrderRequest src, RecOrder upOrder, RecOrdVer upOrdVer, RecOrdVer downOrdVer, TradeBase mktTrade ) {
        ...
        if ( mktTrade.getMultiLegReportingType() != MultiLegReportingType.LegOfSpread ) {
            ...
            double leavesQty = src.getOrderQty() - upOrder._cumQty;

            if ( leavesQty <= Constants.TICK_WEIGHT && !upOrdVer._status.getIsTerminal() ) {
            ...
            } else if ( upOrder._cumQty > Constants.TICK_WEIGHT && (upOrdVer._status == null || upOrdVer._status == OrdStatus.New) ) {
    ...
    private void queueDownstream( Event m, EventHandler dest ) {
        if ( _debug ) {
    ...
    private void queueUpstream( Event m, EventHandler dest ) {
    ...
    private CommonExecRpt regenerateAnExec( DMARecoverySessionContext ctx, RecOrder order, RecOrdVer ordVer, ReusableString buf, RecExec e ) {
        Event m = ctx.regenerate( e._persistKey );
    ...
    private CommonExecRpt regenerateDownExec( RecOrder downOrder, RecOrdVer downOrdVer, ReusableString buf, RecExec e ) {
        DMARecoverySessionContext ctx = _inSessCtx.get( downOrder._sess );
        return regenerateAnExec( ctx, downOrder, downOrdVer, buf, e );
    }

    private BaseOrderRequest regenerateOrdRequest( Map<RecoverableSession, DMARecoverySessionContext> sessCtx, RecoverableSession sess, RecOrdVer ordVer ) {
        DMARecoverySessionContext ctx      = sessCtx.get( sess );
        Event                     regenMsg = ctx.regenerate( ordVer._srcPersistOrderReqKey );
    ...
    private CommonExecRpt regenerateUpExec( RecOrder upOrder, RecOrdVer upOrdVer, ReusableString buf, RecExec e ) {
        DMARecoverySessionContext ctx = _outSessCtx.get( upOrder._sess );
    ...
    private void setLinkId( ZString linkId, long persistKey, DMARecoverySessionContext ctx, ReusableString downClOrdId, RecOrdVer downVersion ) {
        if ( linkId.length() == 0 ) {
    ...
    private void tradeStatusAdjust( OrderRequest src, RecOrder upOrder, RecOrdVer upOrdVer ) {
        ...
        if ( leavesQty > Constants.TICK_WEIGHT ) {
            ...
            if ( upOrder._cumQty > Constants.TICK_WEIGHT ) {
                upOrdVer._status = OrdStatus.PartiallyFilled;
    ...
    private void upstreamInHandleCancelReplaceRequest( CancelReplaceRequest msg, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
        ZString   prevClOrdId = msg.getOrigClOrdId();
        ZString   newclOrdId  = msg.getClOrdId();
    ...
    private void upstreamInHandleCancelRequest( CancelRequest msg, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
        ZString   prevClOrdId = msg.getOrigClOrdId();
        ZString   newclOrdId  = msg.getClOrdId();
        RecOrder  order       = getOrder( _idToUpstreamOrder, prevClOrdId );
        RecOrdVer curVersion  = getVersion( order, prevClOrdId );
    ...
    private void upstreamInHandleNewOrderSingle( NewOrderSingle msg, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
    ...
    private void upstreamOutHandleCancelReject( CancelReject msg, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
        ZString   clOrdId = msg.getClOrdId();
    ...
    private void upstreamOutHandleCancelled( Cancelled msg, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
        upstreamOutReqReply( msg.getOrigClOrdId(), msg, OrdStatus.Canceled, persistKey, ctx, persistFlags );
    }

    private void upstreamOutHandleNewOrderAck( NewOrderAck msg, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
        upstreamOutReqReply( null, msg, OrdStatus.New, persistKey, ctx, persistFlags );
    }

    private void upstreamOutHandleOtherExec( CommonExecRpt msg, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
        ZString   clOrdId = msg.getClOrdId();
        RecOrder  order   = getOrder( _idToUpstreamOrder, clOrdId );
        RecOrdVer version = getVersion( order, clOrdId );
    ...
    private void upstreamOutHandleRejected( Rejected msg, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
        upstreamOutReqReply( msg.getOrigClOrdId(), msg, OrdStatus.Rejected, persistKey, ctx, persistFlags );
    }

    private void upstreamOutHandleReplaced( Replaced msg, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
        upstreamOutReqReply( msg.getOrigClOrdId(), msg, OrdStatus.Replaced, persistKey, ctx, persistFlags );
    }

    private void upstreamOutHandleVagueReject( VagueOrderReject msg, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
        // @TODO handle vague reject
    }

    private void upstreamOutReqReply( ZString origClOrdId, CommonExecRpt msg, OrdStatus status, long persistKey, DMARecoverySessionContext ctx, short persistFlags ) {
        ZString clOrdId = msg.getClOrdId();
