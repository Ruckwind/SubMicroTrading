...
import com.rr.core.collections.EventLinkedList;
...
public class EventProcessorImpl implements EventProcessor {
    ...
    private final EventLinkedList _upStreamQ;
    private final EventLinkedList _downStreamQ;
    ...
    private final EventHandler    _hub;   // message handler for sending alerts to the HUB session
    ...
    @Override
    public final void handle( Event msg ) {
    ...
    @Override
    public final void handleNow( Event msg ) {
    ...
    @Override
    public final void enqueueDownStream( Event msg ) {

        _downStreamQ.add( msg );
    }

    @Override
    public final void enqueueUpStream( Event msg ) {

        _upStreamQ.add( msg );
    }

    @Override
    public void freeMessage( Event msg ) {
        _eventRecycler.recycle( msg );
    }
    ...
    public void routeMessageDownstream( final Order order, final NewOrderSingle nos ) throws StateException {
        ...
        EventHandler handler = _router.getRoute( nos, this );
    ...
    protected EventLinkedList createEventQueue() {
        return new EventLinkedList();
    }
    ...
    private void flush( final EventLinkedList q ) {
    ...
    private void forceCancelToMKt( EventHandler h, ViewString clOrdId, Side side, ViewString orderId ) {
        final CancelRequest mktCancel = _eventBuilder.createForceMarketCancel( clOrdId, side, orderId, null );

        mktCancel.setEventHandler( h );
    ...
    private void handleUnknownExecToHub( CommonExecRpt msg ) {
        ...
        if ( openOnMkt && _config.isForceCancelUnknownExexId() ) {
            ...
            EventHandler h = msg.getEventHandler();
            ...
            if ( h != this && h != null ) {
                ...
                EventHandler[] allRoutes = _router.getAllRoutes();

                for ( EventHandler route : allRoutes ) {
    ...
    private void logMissingHandler( Event msg ) {
        final ReusableString tmpText = TLC.instance().pop();
