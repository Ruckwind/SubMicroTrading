...
public abstract class BaseSMTMain {
    ...
    private static final Logger     _console             = ConsoleFactory.console( BaseSMTMain.class, Level.info );
    ...
    private final Map<String, ControlThread>                  _controllerThreads    = new HashMap<>();
    private final Map<String, MultiSessionThreadedDispatcher> _multiSessDispatchers = new HashMap<>();
    private final Map<String, MultiSessionThreadedReceiver>   _multiSessReceivers   = new HashMap<>();
    ...
    public static void prepare( String[] args, ThreadPriority main ) throws Exception {
        ...
        ThreadUtilsFactory.get().init( props.getProperty( CoreProps.CPU_MASK_FILE ) );
        ThreadUtilsFactory.get().setPriority( Thread.currentThread(), main );

        Level level = PropertyHelper.getProperty( props, CoreProps.LOG_LEVEL, Level.class, Level.info );

        LoggerFactory.setForceConsole( false ); // THIS ISNT LOG LEVEL
        String logFile = getLogFile();

        int minFlushPeriodMS = props.getIntProperty( CoreProps.MIN_LOG_FLUSH_SECS, false, 30 ) * 1000;
        int maxLogSize       = props.getIntProperty( CoreProps.MAX_LOG_SIZE, false, 10000000 );
        LoggerFactory.initLogging( logFile, maxLogSize, level, 131072, null, minFlushPeriodMS );

        setupExchangeManager( props );

        AdminAgent.init( props.getIntProperty( CoreProps.ADMIN_HTML_PORT, false, 8000 ) );
    ...
    private static String getLogFile() {
        ...
        String logRoot = props.getProperty( CoreProps.LOG_ROOT, false, "./logs" );
    ...
    protected RecoverableSession createCMEFastFixSession( String sessName,
                                                          EventDispatcher sendDispatcher,
                                                          SessionManager sessMgr,
                                                          EventHandler proc,
                                                          PropertyGroup propGroup ) {

        int              port         = propGroup.getIntProperty( Tags.port, true, 0 );
        FastSocketConfig socketConfig = new FastSocketConfig( AllEventRecycler.class, false, new ViewString( "localhost" ), null, port );
        ...
        if ( proc == null ) proc = new DummyEventHandler();
     ...
     */
    protected void createCMEFastFixSessionsViaBuilder( EventHandler[] handlers ) {
    ...
    protected RecoverableSession createCMEMultiFastFixSession( String sessName,
                                                               SessionManager sessMgr,
                                                               EventHandler proc,
                                                               PropertyGroup propGroup ) {
        ...
        EventRouter inboundRouter = new PassThruRouter( "passThru", proc ); // currently only using single processor
        ...
        EventQueue                   queue = getQueue( propGroup, sessName );
    ...
    protected RecoverableSession createETISession( CodecId codecId,
                                                   ...
                                                   PropertyGroup propGroup,
                                                   EventDispatcher recSendDispatcher ) throws FileException, SessionException, PersisterException {
        ...
        Decoder        fullDecoder      = getFullDecoder( codecId, client );
        ThreadPriority receiverPriority = getProperty( propGroup, Tags.inThreadPriority, ThreadPriority.class );
        ...
        EventRouter    recInboundRouter = new PassThruRouter( "passThru", proc );
        ...
        Decoder        recFullDecoder   = getFullDecoder( codecId, client );
        ...
        ETISocketSession gwySess = new ETISocketSession( gwySessName, recInboundRouter, gwyConfig, recSendDispatcher,
                                                         recEncoder, recDecoder, recFullDecoder, lowPriority );
    ...
    protected RecoverableSession createFixSession( CodecId codecId,
                                                   ClientProfile client,
                                                   String sessName,
                                                   boolean isDownstream,
                                                   EventDispatcher sendDispatcher,
        ...
        FixDecoder     fullDecoder      = (FixDecoder) getFullDecoder( codecId, client );
        ThreadPriority receiverPriority = getProperty( propGroup, Tags.inThreadPriority, ThreadPriority.class );
        ThreadPriority memmapPriority   = getProperty( propGroup, Tags.persistThreadPriority, ThreadPriority.class );
    ...
    protected RecoverableSession createMilleniumSession( CodecId codecId,
                                                         ClientProfile client,
                                                         String sessName,
                                                         boolean isDownstream,
                                                         EventDispatcher sendDispatcher,
    ...
    protected RecoverableSession createMultiFixSession( CodecId codecId,
                                                        ClientProfile client,
                                                        String sessName,
                                                        boolean isDownstream,
                                                        SessionManager sessMgr,
        ...
        sess = createNonBlockingFixSession( socketConfig, inboundRouter, encoder, decoder, fullDecoder, sessName, dispatcher, receiver, queue );
    ...
    protected RecoverableSession createSession( String sessName,
                                                ClientProfile client,
                                                boolean isDownstream,
                                                SessionManager sessMgr,
                                                EventHandler proc,
        ...
        if ( sessionType == SessionType.FIX ) {
            ...
            EventDispatcher sendDispatcher = getSessionDispatcher( propGroup, sessName + "DISPATCHER", sendPriority );
            ...
            EventDispatcher recoverySendDispatcher = new SessionThreadedDispatcher( sessName + "Recovery", new BlockingSyncQueue(), ThreadPriority.Other );
    ...
    protected RecoverableSession createUTPSession( CodecId codecId,
                                                   ClientProfile client,
                                                   String sessName,
                                                   boolean isDownstream,
                                                   EventDispatcher sendDispatcher,
    ...
    protected EventDispatcher getProcessorDispatcher( PropertyGroup propGroup,
                                                      EventQueue queue,
                                                      String name,
    ...
    protected EventQueue getQueue( PropertyGroup propGroup, String name ) {
        ...
        if ( queueType != null ) {
            if ( queueType.equals( "NonBlockingSync" ) ) {
            ...
            } else if ( queueType.equalsIgnoreCase( "BlockingFixedSize" ) ) {
                int queueSize = propGroup.getIntProperty( Tags.queuePresize, false, 65536 );
                queue = new ArrayBlockingEventQueue( name + "Q", queueSize, false );
            } else if ( queueType.equalsIgnoreCase( "UNSAFE" ) ) {
                queue = new SimpleEventQueue();
            } else if ( queueType.equalsIgnoreCase( "SunCAS" ) ) {
                queue = new JavaConcEventQueue();
            } else if ( queueType.equalsIgnoreCase( "BlockingReusableLinkedQueue" ) ) {
                queue = new ConcBlockingSyncLinkedQueue();
                ...
                int queueSize = propGroup.getIntProperty( Tags.queuePresize, false, 65536 );
                queue = new RingBufferEventQueue1C1P( queueSize );
            } else if ( queueType.equalsIgnoreCase( "RingBuffer1C" ) ) {
                int queueSize = propGroup.getIntProperty( Tags.queuePresize, false, 65536 );
                queue = new RingBufferEventQueueSingleConsumer( queueSize );
            } else {
    ...
    protected void loadSessionSharedComponents() {
        ...
        for ( String controlId : multiControlThreads ) {
            ...
            case TriElement:
                ...
                break;
            case SlowSingleElement:
                ctlThread = new SlowSingleElementControlThread( controlId, priority );
                break;
            case SlowDualElement:
                ctlThread = new SlowDualElementControlThread( controlId, priority );
                break;
            case SlowTriElement:
                ctlThread = new SlowTriElementControlThread( controlId, priority );
                break;
            default:
    ...
    private RecoverableSession createJMXSession( CodecId codecId,
                                                 ClientProfile client,
                                                 String sessName,
                                                 SessionManager sessMgr,
                                                 EventHandler proc,
    ...
    private Decoder getFullDecoder( CodecId id, ClientProfile client ) {
    ...
    private EventDispatcher getSessionDispatcher( PropertyGroup propGroup, String name, ThreadPriority priority ) {
    ...
    private void loadInstrumentStore() throws FileException {
        ...
        if ( instFile != null && !storeType.equalsIgnoreCase( "dummy" ) ) {
            ZString exchangeMIC = new ViewString( instGroup.getProperty( OMProps.Tags.MIC ) );

            if ( storeType.equalsIgnoreCase( "multiExchange" ) ) {
                _instrumentStore = new MultiExchangeInstrumentStore( 1000 );
                ((MultiExchangeInstrumentStore) _instrumentStore).setTickManager( new TickManager( "DefaultTickMgr" ) );
                ...
                Exchange e = ExchangeManager.instance().getByMIC( new ViewString( exchangeMIC ) );

                if ( e == null ) {
                    throw new SMTRuntimeException( "Instrument store REC not in exchange manager : [" + exchangeMIC + "]" );
                }

                _instrumentStore = new SingleExchangeInstrumentStore( e, 1000 );
                ((SingleExchangeInstrumentStore) _instrumentStore).setTickManager( new TickManager( "DefaultTickMgr" ) );

                _console.info( "Loading SingleExchangeInstrumentStore with instrument file " + instFile + ", srcRec=" + exchangeMIC );
            } else {
    ...
    private void registerExchangeSession( SessionManager sessMgr, String propertyGroup, PropertyGroup propGroup, RecoverableSession sess ) {
        String exchangeMIC = AppProps.instance().getProperty( propertyGroup + OMProps.Tags.MIC, false, null );

        if ( exchangeMIC != null ) {
            Exchange e = ExchangeManager.instance().getByMIC( new ViewString( exchangeMIC ) );

            if ( e == null ) {
                throw new SMTRuntimeException( "Session " + propGroup + " exchangeMIC=" + exchangeMIC + " exchange not registered with manager" );
            }
