...
public class LoggedOutFixState implements SessionState {
    ...
    public LoggedOutFixState( RecoverableSession session, FixController engine, FixConfig config ) {
        _session          = session;
        _fixEngine        = engine;
    ...
    public void handle( Event msg ) throws SessionStateException {
        if ( msg.getReusableType().getSubId() == EventIds.ID_LOGON ) {
            ...
            _session.setConnectionId( _config.getTargetCompId() );

            validateLogon( req );

            int requestedNextOUTMsgSeqNum = req.getNextExpectedMsgSeqNum();

            if ( Utils.isNull( requestedNextOUTMsgSeqNum ) ) requestedNextOUTMsgSeqNum = 0;

            final int recvSeqNum      = req.getMsgSeqNum();
            int       nextExpInSeqNum = _fixEngine.getNextExpectedInSeqNo();

            final boolean isReset = !_fixEngine.isServer() && recvSeqNum == 1 && requestedNextOUTMsgSeqNum == 1;

            if ( recvSeqNum < nextExpInSeqNum ) {
            ...
            if ( _fixEngine.isServer() ) {
                if ( _config.isUseNewFix44GapFillProtocol() && requestedNextOUTMsgSeqNum > 0 ) {
                    _fixEngine.adjustNextOutSeqNum( requestedNextOUTMsgSeqNum );
                    ++requestedNextOUTMsgSeqNum;
                }
                _fixEngine.sendLogonNow( req.getHeartBtInt(), nextExpInSeqNum + 1 );
            }
            ...
            if ( _config.isUseNewFix44GapFillProtocol() && requestedNextOUTMsgSeqNum > 0 ) {
                ...
                if ( !isReset && requestedNextOUTMsgSeqNum < _fixEngine.getNextOutSeqNum() ) {
                    _fixEngine.sendMissingMsgsToClientNow( requestedNextOUTMsgSeqNum );
                } else {
                    _logMsg.copy( _session.id() ).append( " logon reply from server resets seqNums to 1" );
                    _fixEngine.adjustNextOutSeqNum( requestedNextOUTMsgSeqNum );
                }
            }
    ...
    private void acceptMesssage( Event msg ) {
    ...
    private void validateLogon( Logon msg ) throws SessionStateException {
        ...
        if ( !msg.getTargetCompId().equals( fsc.getSenderCompId() ) ) {
            _logMsg.copy( INVALID_TARGET_COMP_ID ).append( msg.getTargetCompId() ).append( " expected " ).append( fsc.getSenderCompId() );
            _fixEngine.sendLogoutNow( _logMsg, msg );
            throw new SessionStateException( _logMsg.toString() );
        }

        if ( !msg.getSenderCompId().equals( fsc.getTargetCompId() ) ) {
            _logMsg.copy( INVALID_SENDER_COMP_ID ).append( msg.getSenderCompId() ).append( " expected " ).append( fsc.getTargetCompId() );
            _fixEngine.sendLogoutNow( _logMsg, msg );
            throw new SessionStateException( _logMsg.toString() );
        }
