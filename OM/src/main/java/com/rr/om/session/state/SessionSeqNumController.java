...
public class SessionSeqNumController extends SessionController<StatefulSessionFactory> {
    ...
    private volatile int _nextExpectedInSeqNo = 1;
    private volatile int _nextOutSeqNo        = 1;
    private          int _lastBeginSeqNo      = -1;
    private ReusableString _logOutMsg = new ReusableString( 100 );
    private volatile boolean _forceSeqNumReset = false;
    ...
    @Override
    public void recoverContext( Event msg, boolean inBound ) {
    ...
    protected Event formLogoutMessage() {
        Event lo = _sessionFactory.getLogOut( ERR_MISSED_HB.getError(), 0, null, _nextOutSeqNo, _nextExpectedInSeqNo );
        return lo;
    }
    ...
    public final void clientResyncOutEvents( int beginSeqNo, int endSeqNo, boolean sendImmediately ) {
        ...
        if ( _forceSeqNumReset ) {
            ...
            final Event gap = _sessionFactory.createForceSeqNumResetMessage( _nextOutSeqNo + 1 );
        ...
        for ( int curMsgSeqNo = beginSeqNo; curMsgSeqNo <= endSeqNo; curMsgSeqNo++ ) {
            ...
            final Event recMsg = seqSession.retrieve( curMsgSeqNo );
        ...
        if ( gapBeginSeqNo != -1 ) {
            if ( seqSession.getStateConfig().isGapFillAllowed() ) {
                msgsSent += sendGapMsgs( sendImmediately, gapBeginSeqNo, endSeqNo );
            } else {
    ...
    public void forceLogOn() {
        ...
        if ( _session.isConnected() ) {
            ...
            _session.requestConnect();
        }
    }

    public void forceLogOut() {
        LogoutImpl logOut = new LogoutImpl();
        logOut.setEventHandler( _session );
    ...
    public void sendLogonNow( int heartBtInt, int nextExpInSeqNum ) {
        Event logon = _sessionFactory.getSessionLogOn( heartBtInt, _nextOutSeqNo, nextExpInSeqNum );
    ...
    public void sendLogoutNow( ReusableString logMsg, Event logon ) {
        Event hb = _sessionFactory.getLogOut( logMsg, 0, logon, _nextOutSeqNo, _nextExpectedInSeqNo );

        _session.handleNow( hb );

        boolean tryReconnect = !isServer(); // server session should NOT pause if get disconnect
    ...
    public void sendMissingMsgsToClientNow( int fromSeqNum ) {
        clientResyncOutEvents( fromSeqNum, 0, true );
    }
    ...
    private void doSendResendRequestNow( int fromSeqNum, int toSeqNum, final boolean isNow ) {
        Event req = _sessionFactory.getResendRequest( fromSeqNum, toSeqNum );
        if ( isNow ) {
    ...
    private int sendDummyMsgs( boolean sendImmediately, int gapBeginSeqNo, int gapEndSeqNo ) {
        ...
        for ( int seqNum = gapBeginSeqNo; seqNum <= gapEndSeqNo; ++seqNum ) {
            final Event msg = _sessionFactory.getHeartbeat( null );
    ...
    private int sendGapMsgs( boolean sendImmediately, int gapBeginSeqNo, int gapEndSeqNo ) {
        int         msgsSent = 0;
        final Event gap      = _sessionFactory.createGapFillMessage( gapBeginSeqNo, gapEndSeqNo );
        if ( gap != null ) {
            send( gap, sendImmediately );
            ++msgsSent;
        }
