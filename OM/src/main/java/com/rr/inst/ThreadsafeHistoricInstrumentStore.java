...
public final class ThreadsafeHistoricInstrumentStore implements HistInstStore {

    private final     OptimisticReadWriteLock _stampLock = new StampedLockProxy();
    private final     HistInstStore           _store;
    private transient RunState                _runState  = RunState.Unknown;

    public ThreadsafeHistoricInstrumentStore( HistInstStore store ) {
        _store = store;
    }
    ...
    @Override public void getAllInstruments( final Set<ExchInstSecDefWrapperTSEntry> instruments, final long atTimestamp ) {
        long stamp = grabWriteLock();
        try {
            _store.getAllInstruments( instruments, atTimestamp );
        } finally {
            releaseWriteLock( stamp );
        }
    }

    @Override public CommonInstrument getCommonInstrument( final long commonInstrumentId, final Currency ccy, final long atTimestamp ) {
        long stamp = grabWriteLock();
        try {
            return _store.getCommonInstrument( commonInstrumentId, ccy, atTimestamp );
        } finally {
            releaseWriteLock( stamp );
        }
    }

    @Override public TimeSeries<CommonInstrument> getCommonInstrumentSeries( final long commonInstrumentId, final Currency ccy ) {
        long stamp = grabWriteLock();
        try {
            return _store.getCommonInstrumentSeries( commonInstrumentId, ccy );
        } finally {
            releaseWriteLock( stamp );
        }
    }

    @Override public void getCommonInstruments( final long commonInstrumentId, final long atTimestamp, final List<CommonInstrument> dest ) {
        long stamp = grabWriteLock();
        try {
            _store.getCommonInstruments( commonInstrumentId, atTimestamp, dest );
        } finally {
            releaseWriteLock( stamp );
        }
    }

    @Override public ExchInstSecDefWrapperTSEntry getExchInstAt( final ZString securityId, final SecurityIDSource securityIDSource, final long atTimestamp ) {
        long stamp = grabWriteLock();
        try {
            return _store.getExchInstAt( securityId, securityIDSource, atTimestamp );
        } finally {
            releaseWriteLock( stamp );
        }
    }

    @Override public ExchInstSecDefWrapperTSEntry getExchInstAt( final ZString securityId, final SecurityIDSource securityIDSource, final ExchangeCode exchangeCode, final long atTimestamp ) {
        long stamp = grabWriteLock();
        try {
            return _store.getExchInstAt( securityId, securityIDSource, exchangeCode, atTimestamp );
        } finally {
            releaseWriteLock( stamp );
        }
    }

    @Override public ExchInstSecDefWrapperTSEntry getExchInstAt( final ZString securityId, final SecurityIDSource securityIDSource, final ExchangeCode exchangeCode, final Currency currency, final long atTimestamp ) {
        long stamp = grabWriteLock();
        try {
            return _store.getExchInstAt( securityId, securityIDSource, exchangeCode, currency, atTimestamp );
        } finally {
            releaseWriteLock( stamp );
        }
    }

    @Override public TimeSeries<ExchInstSecDefWrapperTSEntry> getExchInstSeriesByUniqueInstId( final long uniqueInstId ) {
        long stamp = grabWriteLock();
        try {
            return _store.getExchInstSeriesByUniqueInstId( uniqueInstId );
        } finally {
            releaseWriteLock( stamp );
        }
    }

    @Override public HistExchInstSecDefWrapperTS getExchInstTS( final long longKey, final SecurityIDSource securityIDSource ) {
        long stamp = grabWriteLock();
        try {
            return _store.getExchInstTS( longKey, securityIDSource );
        } finally {
            releaseWriteLock( stamp );
        }
    }

    @Override public HistExchInstSecDefWrapperTS getExchInstTS( final long longKey, final ExchangeCode exchangeCode ) {
        long stamp = grabWriteLock();
        try {
            return _store.getExchInstTS( longKey, exchangeCode );
        } finally {
            releaseWriteLock( stamp );
        }
    }

    @Override public HistExchInstSecDefWrapperTS getExchInstTS( final ZString securityId, final SecurityIDSource securityIDSource ) {
        long stamp = grabWriteLock();
        try {
            return _store.getExchInstTS( securityId, securityIDSource );
        } finally {
            releaseWriteLock( stamp );
        }
    }

    @Override public HistExchInstSecDefWrapperTS getExchInstTS( final ZString securityId, final SecurityIDSource securityIDSource, final ExchangeCode exchangeCode ) {
        long stamp = grabWriteLock();
        try {
            return _store.getExchInstTS( securityId, securityIDSource, exchangeCode );
        } finally {
            releaseWriteLock( stamp );
        }
    }

    @Override public HistExchInstSecDefWrapperTS getExchInstTS( final ZString securityId, final SecurityIDSource securityIDSource, final ExchangeCode exchangeCode, final Currency currency ) {
        long stamp = grabWriteLock();
        try {
            return _store.getExchInstTS( securityId, securityIDSource, exchangeCode, currency );
        } finally {
            releaseWriteLock( stamp );
        }
    }

    @Override public void getInstruments( final Set<ExchInstSecDefWrapperTSEntry> instruments, final Exchange ex, final long atTimestamp ) {
        long stamp = grabWriteLock();
        try {
            _store.getInstruments( instruments, ex, atTimestamp );
        } finally {
            releaseWriteLock( stamp );
        }
    }

    @Override public ParentCompany getParentCompany( final long parentCompanyId, final long atTimestamp ) {
        long stamp = grabWriteLock();
        try {
            return _store.getParentCompany( parentCompanyId, atTimestamp );
        } finally {
            releaseWriteLock( stamp );
        }
    }

    @Override public TimeSeries<ParentCompany> getParentCompanySeries( final long parentCompanyId ) {
        long stamp = grabWriteLock();
        try {
            return _store.getParentCompanySeries( parentCompanyId );
        } finally {
            releaseWriteLock( stamp );
        }
    }
