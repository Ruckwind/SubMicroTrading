...
static char* OUT_MCAST_GRP = "226.1.1.1";
static char* OUT_NIC_IP    = "192.168.10.2";
static int   OUT_PORT      = 4321;

static char* IN_MCAST_GRP = "226.1.1.2";
static char* IN_NIC_IP    = "192.168.10.2";
static int   IN_PORT       = 4322;
...
void dosend( unsigned int idx, int sd, char* databuf, int datalen, void* grpSockPtr ) {
	struct sockaddr_in* groupSock = (struct sockaddr_in*) grpSockPtr;

	unsigned long nanos = getNow();

	unsigned char *t = databuf;

    *(t++) = (unsigned char) (idx >> 24);
    *(t++) = (unsigned char) (idx >> 16);
    *(t++) = (unsigned char) (idx >> 8);
    *(t++) = (unsigned char) (idx >> 0);

    *(t++) = (unsigned char) (nanos >> 56);
    *(t++) = (unsigned char) (nanos >> 48);
    *(t++) = (unsigned char) (nanos >> 40);
    *(t++) = (unsigned char) (nanos >> 32);
    *(t++) = (unsigned char) (nanos >> 24);
    *(t++) = (unsigned char) (nanos >> 16);
    *(t++) = (unsigned char) (nanos >> 8);
    *(t++) = (unsigned char) (nanos >> 0);
...
while( idx < 10000 ) {
	dosend( idx, outSD, bufOUT, datalen, groupSockOUT );
