...
static char* OUT_IP   = "192.168.10.1";
...
void dosend( unsigned int idx, int sd, char* databuf, int datalen ) {
    ...
    *(t++) = (unsigned char) (nanos >> 0);
	...
	while( bytesToWrite > 0 ) {
		if((written=write(sd, databuf, datalen)) < 0) {
...
while( idx < 10000 ) {
	dosend( idx, inSD, bufOUT, datalen );
