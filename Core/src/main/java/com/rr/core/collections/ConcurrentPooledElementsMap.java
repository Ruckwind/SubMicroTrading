...
public class ConcurrentPooledElementsMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
    ...
    public static final class ConcHashEntry<K, V> implements Reusable<ConcHashEntry<K, V>> {
        ...
        static <K, V> ConcHashEntry<K, V>[] newArray( int i ) {
    ...
    static final class Segment<K, V> extends ReentrantLock implements Serializable {
         ...
         * that could even momentarily cause a concurrent read operation to see inconsistent lva. This is made easier by the nature of the read operations in
         * Map. For sample, no operation can reveal that the table has grown but the threshold has not yet been updated, so there are no atomicity requirements
        ...
        private final ConcHashEntryFactory<K, V>    _entryFactory;
        private final Recycler<ConcHashEntry<K, V>> _entryRecycler;
        /**
        ...
        Segment( int initialCapacity, float lf ) {
            ...
            _entryFactory  = SuperpoolManager.instance().getFactory( ConcHashEntryFactory.class, ConcHashEntry.class );
            entryRecycler  = SuperpoolManager.instance().getRecycler( ConcHashEntry.class );
            _entryRecycler = (Recycler<ConcHashEntry<K, V>>) entryRecycler;
     ...
     */
    @SuppressWarnings( { "serial" } ) final class WriteThroughEntry extends AbstractMap.SimpleEntry<K, V> {
