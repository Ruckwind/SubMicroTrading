...
public class SuperPool<T extends Reusable<T>> {
    ...
    private static final Logger _log = ConsoleFactory.console( SuperPool.class, Level.WARN );

    // TODO benchmark to see optimal size

    // TODO register shutdown callback to logger stats
     ...
     * Some lists have a corresponding last pointer, if lastPointer == root then there is no lva,
     ...
     */
    private final PoolNode<T> _readyChains = new PoolNode<>();
     ...
     */
    private final PoolNode<T> _returnedChains    = new PoolNode<>();
    /**
     ...
     */
    private final PoolNode<T> _freeReadyNodes    = new PoolNode<>();
    /**
     ...
     */
    private final PoolNode<T> _freeReturnNodes = new PoolNode<>();
    ...
    public T getChain() {
        ...
        PoolNode<T> node;
        PoolNode<T> returnEmptyNodes;
        PoolNode<T> lastReturnEmptyNodes;
        ...
        PoolNode<T> lastPopulatedChainNode;
    ...
    /**
     * @return a generic pool recycler ... WILL NOT recycle any sub elements, for that you must use the generated event specific recyclers
     */
    public Recycler<T> getRecycleFactory() {
        return new RecyclerImpl<>( _poolClass, _chainSize, this );
    }
    ...
    public void returnChain( T list ) {
        ...
        PoolNode<T> emptyNodeChain;
        PoolNode<T> lastEmptyNode;
