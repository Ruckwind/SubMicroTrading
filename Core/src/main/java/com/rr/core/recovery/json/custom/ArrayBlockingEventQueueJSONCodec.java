...
public class ArrayBlockingEventQueueJSONCodec implements JSONClassCodec {
    ...
    @Override public void encode( final JSONWriter writer, final Object val, int objectId ) throws Exception {
        ...
        ArrayBlockingEventQueue coll = (ArrayBlockingEventQueue) val;

        startObjectPrepForNextField( writer, val, objectId, true, PersistMode.AllFields, null );

        writer.write( writer.getWorkStr().copy( "\"" ).append( CAPACITY ).append( "\" : " ).append( coll.maxCapacity() ) );
    ...
    @Override public Object decode( final JSONReader reader, final ReusableString tmpStr, Class<?> postClass, int jsonId ) throws Exception {
        ...
        if ( !tmpStr.equals( CAPACITY ) ) throw new JSONException( "ArrayBlockingQueueJSONCodec.decode .. expected '" + CAPACITY + "' not " + tmpStr );
        tokeniser.nextToken( JSONInputTokeniser.Token.Colon );
        int size = reader.getInt();

        ArrayBlockingEventQueue coll = new ArrayBlockingEventQueue( "Q", size, false );
