...
public abstract class AbstractSocketSession extends AbstractSession {
    ...
    private static final   int  MAX_FAIL_BLOCK_CONNECTS           = 5;
    private static final   int  FAIL_BLOCK_CONNECT_SLEEP_MS       = 5000;
    ...
    /**
     * buffer for receiving messages, to avoid extra mempcy on logger place the logger hdr at start of buffer
     */
    protected final ByteBuffer     _inNativeByteBuffer;
    protected final byte[]         _inBuffer;
    protected final ByteBuffer     _inByteBuffer;
    protected final ViewString     _inLogBuf;
    ...
    protected final ByteBuffer     _outNativeByteBuffer;
    ...
    private boolean _firstConnErr       = true;
    private volatile int  _nextRetryDelayMS   = SessionConstants.CONNECT_WAIT_DELAY_MS;
    ...
    private volatile int  _connectAttempts    = 0;
    ...
    public AbstractSocketSession( String name,
        ...
        super( name, inboundRouter, socketConfig, dispatcher, encoder, decoder, fullDecoder );

        _inNativeByteBuffer  = ByteBuffer.allocateDirect( socketConfig.getCodecBufferSize() );
        _outNativeByteBuffer = ByteBuffer.allocateDirect( socketConfig.getCodecBufferSize() );

        _inBuffer     = new byte[ socketConfig.getCodecBufferSize() ];
        _inByteBuffer = ByteBuffer.wrap( _inBuffer );
        _inLogBuf     = new ViewString( _inBuffer );
        ...
        if ( maxConnAttempts > SessionConstants.MAX_CONNECT_ATTEMPTS ) maxConnAttempts = SessionConstants.MAX_CONNECT_ATTEMPTS;

        _maxConnectAttempts = maxConnAttempts;
    ...
    public synchronized void connect() {
        _log.info( "Session [" + getIntId() + "] " + getComponentId() + " connect invoked, receiver=" + ((_receiver == null) ? "<null>" : _receiver.getComponentId()) );
    ...
    public void internalConnect() {
        ...
        if ( getSessionState() == SessionState.Disconnected ) {
            ...
            if ( _socketConfig.isOpenUTC( ClockFactory.get().currentTimeMillis() ) ) {
    ...
    @Override public void requestConnect() {
        if ( getSessionState() == SessionState.Disconnected ) {
            ...
            synchronized( this ) {
                _log.info( "Session " + getComponentId() + " request to connect, wake receiver " );
                _loggedNotInSession = false;
                _connectAttempts    = 0;
                _nextRetryDelayMS   = 100;
    ...
    public void waitConnect( String key ) {
        ...
        while( getSessionState() != SessionState.Connected ) {

            ThreadUtilsFactory.getLive().waitFor( this, SessionConstants.CONNECT_WAIT_DELAY_MS );
        }

        _log.info( "Session " + getComponentId() + " " + key + " confirm connected" );
    }

    protected void bindServerInterface( LiteServerSocket serverSocketChannel ) throws IOException {
        ...
        do {
            if ( _socketChannel.write() == 0 ) {
                if ( failWriteCount++ >= MAX_SPIN_FAIL_WRITES_MICROS ) {
                    ThreadUtilsFactory.get().sleepMicros( nextFailDelay );
    ...
    protected final Event decode( int offset, int len ) {

        return _decoder.decode( _inBuffer, offset, len + offset );
    }

    protected final int getConnectAttempts()                                       { return _connectAttempts; }

    protected final void setConnectAttempts( final int connectAttempts )           { _connectAttempts = connectAttempts; }
    ...
    protected final boolean isLoggedNotInSession()                                 { return _loggedNotInSession; }

    protected final void setLoggedNotInSession( final boolean loggedNotInSession ) { _loggedNotInSession = loggedNotInSession; }
    ...
    protected LiteSocket serverNIOConnect() {
    ...
    protected void setSocketOptions( LiteSocket channel ) throws SocketException {
        ...
        if ( _socketConfig.getKeepAlive() != channel.getKeepAlive() ) {
            _log.info( "Note socket " + getComponentId() + " setting KeepAlive to " + _socketConfig.getKeepAlive() );
            channel.setKeepAlive( _socketConfig.getKeepAlive() );
        }
        ...
        if ( _socketConfig.getReuseAddress() != channel.getReuseAddress() ) {
            _log.info( "Note socket " + getComponentId() + " setting ReuseAddress to " + _socketConfig.getReuseAddress() );
            channel.setReuseAddress( _socketConfig.getReuseAddress() );
        }
    }

    protected void setupChannel( LiteSocket channel ) throws Exception {
        channel.setTcpNoDelay( _socketConfig.getTcpNoDelay() );
    }
    ...
    private synchronized void chkPersistenceOpen() throws PersisterException {
        _inPersister.open();
        _outPersister.open();
    }
    ...
    private LiteSocket clientBlockingConnect() {
        ...
        while( ++i <= MAX_FAIL_BLOCK_CONNECTS ) {
            ...
            try {
                ...
                if ( i == MAX_FAIL_BLOCK_CONNECTS ) {
                    _log.error( SessionConstants.ERR_OPEN_SOCK, getComponentId() + " Failed to connect as client blocking socket: " + ", host=" + _socketConfig.getHostname() + ", port=" + _socketConfig.getPort() + " : " + e.getMessage() );
                } else {
                    _log.info( getComponentId() + " Failed to connect [#" + i + "] as client blocking socket: " + ", host=" + _socketConfig.getHostname() + ", port=" + _socketConfig.getPort() + " : " + e.getMessage()
                               + " will sleep and retry" );

                    ThreadUtilsFactory.get().sleep( FAIL_BLOCK_CONNECT_SLEEP_MS );
                }

            } catch( Exception e ) {
                _log.error( SessionConstants.ERR_OPEN_SOCK, getComponentId() + " Failed to create client blocking socket: " + e.getMessage() + ", host=" + _socketConfig.getHostname() + ", port=" + _socketConfig.getPort(), e );
    ...
    private LiteSocket clientNIOConnect() {
        ...
        try {
            ...
            if ( _firstConnErr ) {
                ...
                _log.info( getComponentId() + " Failed to connect as client NIO socket: " +
                           ", host=" + _socketConfig.getHostname() + ", port=" + _socketConfig.getPort() + " : " + e.getMessage() );
            }
        } catch( Exception e ) {
    ...
    private LiteSocket connectSocket() {
        ...
        while( !isPaused() && !isStopping() && ++_connectAttempts < _maxConnectAttempts ) {
            if ( _socketConfig.isNIO() ) {
            ...
            ThreadUtilsFactory.getLive().waitFor( this, _nextRetryDelayMS );
