...
public class TestMulticastSocketFromFactory extends BaseTestCase {
    ...
    static {
        try {
            DummyAppProperties.testInit( Collections.singletonMap( CoreProps.RUN_ENV, Env.TEST.name() ) );
        } catch( Exception e ) {
            fail( e.getMessage() );
        }
    }
    ...
    private void doRun( int numConsumers, int numProducers, int sendPerProducer, int port ) {
        ...
        ThreadUtilsFactory.get().sleep( 3000 );
        ...
        for ( int i = 0; i < numConsumers; ++i ) {
            assertTrue( consumers[ i ].getIn() > 0 );
        }

        for ( int i = 0; i < numProducers; ++i ) {
            assertTrue( producers[ i ].getOut() > 0 );
        }
    }
