...
public class TestLogging extends BaseTestCase {
    ...
    static class DummyLogDelegate implements Appender {
        ...
        private List<LogEvent> _sent = new ArrayList<>();

        @Override public void chain( final Appender dest ) { /* not used */ }
        ...
        @Override public void init( final Level level ) { _init = Utils.nanoTime(); }
    ...
    public void testAsync() {
        ...
        asyncAppender.init( Level.info );

        LogDelegator delegator = new LogDelegator( asyncAppender, Level.info, this.getClass() );
        ...
        LogEvent firstEvent = getEvent( delegate );
        assertTrue( "First event starts with AsyncAppender.LoggerThread: " + firstEvent.getMessage(), firstEvent.getMessage().contains( "AsyncAppender.LoggerThread" ) );
    ...
    private LogEvent getEvent( DummyLogDelegate delegate ) {
        ...
        do {
            ...
            synchronized( list ) {
                ...
                if ( entries == 0 ) {
                    ThreadUtilsFactory.get().waitFor( list, 100 );
                } else {
            ...
            delay = ClockFactory.get().currentTimeMillis() - start;

        } while( delay < MAX_WAIT );

        assertTrue( "delay " + delay + " < " + MAX_WAIT, delay < MAX_WAIT );
