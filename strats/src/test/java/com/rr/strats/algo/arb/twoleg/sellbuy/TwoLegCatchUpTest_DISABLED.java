...
@Ignore
public class TwoLegCatchUpTest_DISABLED extends BaseStrategyArbTwoLegTst {
    ...
    @Test public void testWhenFullyFilledStateBackToNone() {
        ...
        NewOrderSingle spreadNOS = verifyNOS( 0, _instSpread, 5, 409.5, Side.Sell,  TimeInForce.ImmediateOrCancel );
        NewOrderSingle legOneNOS = verifyNOS( 1, _instLeg1,   5, 155.0, Side.Buy,   TimeInForce.ImmediateOrCancel );
        NewOrderSingle legTwoNOS = verifyNOS( 2, _instLeg2,   5, 252.5, Side.Sell,  TimeInForce.ImmediateOrCancel );
        ...
        sendFill( _spread, spreadNOS, 5, 409.5 );
        sendFill( _leg1,   legOneNOS, 5, 155.0 );
        sendFill( _leg2,   legTwoNOS, 5, 252.5 );
        _strat.doWorkUnit();
        ...
        verifyInstState( _spread, 409.5, 410.0,  0,         0,          0,          5, 0, 0 );
        verifyInstState( _leg1,   152.5, 155.0,  0,         0,          5,          0, 0, 0 );
        verifyInstState( _leg2,   252.5, 257.5,  0,         0,          0,          5, 0, 0 );
        verifyStratState( expProfit );

        // HUB will have the 3 fills as well
        //                       idx, expPnl, algoSeqNum, tickId, lastEventInst
        checkStratStateHubEvent( 3,     10,       1,         1002,       2 );
    }
    ...
    @Test public void testCancelledWithOneIOCOpenStaysInOpenState() {
        ...
        // HUB will have the 3 fills as well
        //                       idx, expPnl, algoSeqNum, tickId, lastEventInst
        checkStratStateHubEvent( 2,     10,       1,         1002,       1 );
    }
    ...
    @Test public void testResubmitIOCWhenArbStillExists() {
        ...
        legOneNOS = verifyNOS( 0, _instLeg1,   5, 155.0, Side.Buy,   TimeInForce.ImmediateOrCancel );
        checkStratCatchupState( _catchupStates.getInitialOpen() );
        ...
        //                       idx, expPnl, algoSeqNum, tickId, lastEventInst
        checkStratStateHubEvent( 3,     0,       1,         1002,       1 );
        ...
        //                       idx, expPnl, algoSeqNum, tickId, lastEventInst
        checkStratStateHubEvent( 1,     expProfit,       1,         1002,       0 );
    }
    ...
    @Test public void testSubmitLimitWhenArbDoesntExist() {
        ...
        clearQueues();
            ...
            mdDeltaTopOfBook( _instSpread, 8,  407.5, MDEntryType.Bid);  // no arb op
            _strat.doWorkUnit();
            assertEquals( 0, events.size() );       
        ...
        clearQueues();
            ...
            _strat.doWorkUnit();
            assertEquals( 1, events.size() );       
            legOneNOS = verifyNOS( 0, _instLeg1,   5, 155.0, Side.Buy,   TimeInForce.Day);  // DAY LIMIT ORDER
            checkStratCatchupState( _catchupStates.getLimitOrderCatchup() );
            verifyInstState( _spread, 407.5, 410.0,  0,         0,          0,          5, 0, 0 );
        ...
        clearQueues();
            ...
            // HUB will have the tradeNew + status
            //                       idx, expPnl, algoSeqNum, tickId, lastEventInst
            checkStratStateHubEvent( 1,     expProfit,       1,         1003,       0 );
    }
    ...
    @Test public void testFlatten() {
        ...
        clearQueues();
            ...
            mdDeltaTopOfBook( _instSpread, 8,  410.5, MDEntryType.Offer );  // no arb op
            _strat.doWorkUnit();
            assertEquals( 0, events.size() );       
            // note the change in book didnt get updated to the stratInstState struct as strat active no recalc performed
            verifyInstState( _spread, 407.5, 410.0,  0,         0,          0,          5, 0, 0 );
        ...
        clearQueues();
            ...
            legTwoNOS = verifyNOS( 0, _instLeg2,   5, 257.5, Side.Buy,  TimeInForce.ImmediateOrCancel ); // BUY to get out position
            spreadNOS = verifyNOS( 1, _instSpread, 5, 410.5, Side.Buy,  TimeInForce.ImmediateOrCancel ); // BUY to get out position

            //                inst,   bidPx, askPx, openLong, openShort, executedLong, executedShort
            verifyInstState( _spread, 407.5, 410.5,  5,         0,          0,          5, 0, 5 );
            verifyInstState( _leg1,   152.5, 155.0,  0,         0,          0,          0, 0, 0 );
            verifyInstState( _leg2,   252.5, 257.5,  5,         0,          0,          5, 0, 5 );
        
        clearQueues();
        
            sendFill( _spread, spreadNOS, 5, 410.5 );
            _strat.doWorkUnit();
            assertEquals( 0, events.size() );       
            checkStratCatchupState( _catchupStates.getFlatten() );

        clearQueues();
            sendFill( _leg2,   legTwoNOS, 5, 257.5 );
            _strat.doWorkUnit();
            assertEquals( 0, events.size() );       
            checkStratCatchupState( _catchupStates.getNone() );
    
            //                inst,   bidPx, askPx, openLong, openShort, executedLong, executedShort, unwindPnl, unfilled
            verifyInstState( _spread, 407.5, 410.5,  0,         0,          5,          5,              -5,        0 );
            verifyInstState( _leg1,   152.5, 155.0,  0,         0,          0,          0,               0,         0 );
            verifyInstState( _leg2,   252.5, 257.5,  0,         0,          5,          5,              -25,        0 );

            // profit = cost to backout leg2 + cost to backout spread
            // leg2    (252.5 * 5) - (257.5 * 5) = 1262.5 - 1287.5 = -25
            // spread  (409.5 * 5) - (410.5 * 5) = 2047.5 - 2052.5 = -5
            // total profit = -35
            ...
            // HUB will have the tradeNew + status
            //                       idx, expPnl, algoSeqNum, tickId, lastEventInst
            checkStratStateHubEvent( 1,     expProfit,       1,         1004,       1 );
    }
