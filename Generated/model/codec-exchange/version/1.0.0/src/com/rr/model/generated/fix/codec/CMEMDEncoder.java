...
public final class CMEMDEncoder implements FixEncoder {
   ...
   // Constructors
    public CMEMDEncoder( String id, byte[] buf, int offset ) {
        this( id, FixVersion.MDFix4_4._major, FixVersion.MDFix4_4._minor, buf, offset );
    }

    public CMEMDEncoder( byte[] buf, int offset ) {
        this( null, FixVersion.MDFix4_4._major, FixVersion.MDFix4_4._minor, buf, offset );
    }

    public CMEMDEncoder( byte major, byte minor, byte[] buf, int offset ) {
        this( null, major, minor, buf, offset );
    }

    public CMEMDEncoder( String id, byte major, byte minor, byte[] buf, int offset ) {
    ...
    public CMEMDEncoder( String id, byte major, byte minor, byte[] buf ) {
        this( id, major, minor, buf, 0 );
    }

    public CMEMDEncoder( byte major, byte minor, byte[] buf ) {
   ...
   // encode methods
    ...
    public final void encodeSecurityStatus( final SecurityStatus msg ) {
        ...
        _builder.encodeString( FixDictionaryMD44.securityID, msg.getSecurityID() );        // tag48
    ...
    public final void encodeSecurityDefinition( final SecurityDefinition msg ) {
        ...
        _builder.encodeInt( FixDictionaryMD44.MsgSeqNum, msg.getMsgSeqNum() );        // tag34
            // skip;
        _builder.encodeLong( FixDictionaryMD44.uniqueInstId, msg.getUniqueInstId() );        // tag9814
        ...
        _builder.encodeString( FixDictionaryMD44.securityLongDesc, msg.getSecurityLongDesc() );        // tag9800

        {
            SecDefEventImpl tmpSecDefEvent = (SecDefEventImpl)msg.getEvents();
            int counterSecDefEvent = msg.getNoEvents();
            _builder.encodeInt( FixDictionaryMD44.noEvents, counterSecDefEvent );
            int i=0;

            while ( tmpSecDefEvent != null ) { 
                ++i;
                final SecDefEventType tEventType = tmpSecDefEvent.getEventType();
                if ( tEventType != null ) _builder.encodeByte( FixDictionaryMD44.eventType, tEventType.getVal() );        // tag865
                _builder.encodeLong( FixDictionaryMD44.eventDate, tmpSecDefEvent.getEventDate() );        // tag866
                _builder.encodeLong( FixDictionaryMD44.eventTime, tmpSecDefEvent.getEventTime() );        // tag1145
...

        {
            ...
            _builder.encodeInt( FixDictionaryMD44.noLegs, counterSecDefLeg );
            int i=0;

            while ( tmpSecDefLeg != null ) { 
                ++i;
                _builder.encodeString( FixDictionaryMD44.legSymbol, tmpSecDefLeg.getLegSymbol() );        // tag600
                _builder.encodeString( FixDictionaryMD44.legSecurityDesc, tmpSecDefLeg.getLegSecurityDesc() );        // tag620
                _builder.encodeInt( FixDictionaryMD44.legRatioQty, tmpSecDefLeg.getLegRatioQty() );        // tag623
                final Side tLegSide = tmpSecDefLeg.getLegSide();
                if ( tLegSide != null ) _builder.encodeByte( FixDictionaryMD44.legSide, tLegSide.getVal() );        // tag624
                _builder.encodeString( FixDictionaryMD44.legSecurityID, tmpSecDefLeg.getLegSecurityID() );        // tag602
                final SecurityIDSource tLegSecurityIDSource = tmpSecDefLeg.getLegSecurityIDSource();
                if ( tLegSecurityIDSource != null ) _builder.encodeByte( FixDictionaryMD44.legSecurityIDSource, tLegSecurityIDSource.getVal() );        // tag603
        ...
        _builder.encodeInt( FixDictionaryMD44.TickRule, msg.getTickRule() );        // tag6350
        _builder.encodePrice( FixDictionaryMD44.futPointValue, msg.getFutPointValue() );        // tag1700
        ...
        if ( tUnitOfMeasure != null ) _builder.encodeBytes( FixDictionaryMD44.unitOfMeasure, tUnitOfMeasure.getVal() );        // tag996
        _builder.encodePrice( FixDictionaryMD44.minPriceIncrementAmount, msg.getMinPriceIncrementAmount() );        // tag1146
        _builder.encodePrice( FixDictionaryMD44.unitOfMeasureQty, msg.getUnitOfMeasureQty() );        // tag1147
        ...
        if ( tUnderlyingProduct != null ) _builder.encodeTwoByte( FixDictionaryMD44.underlyingProduct, tUnderlyingProduct.getVal() );        // tag462
        final ExchangeCode tSecurityExchange = msg.getSecurityExchange();
        if ( tSecurityExchange != null ) _builder.encodeBytes( FixDictionaryMD44.securityExchange, tSecurityExchange.getVal() );        // tag207
        final SecurityIDSource tUnderlyingSecurityIDSource = msg.getUnderlyingSecurityIDSource();
        if ( tUnderlyingSecurityIDSource != null ) _builder.encodeByte( FixDictionaryMD44.underlyingSecurityIDSource, tUnderlyingSecurityIDSource.getVal() );        // tag9801
        _builder.encodeString( FixDictionaryMD44.underlyingSecurityID, msg.getUnderlyingSecurityID() );        // tag9802
        final ExchangeCode tUnderlyingScurityExchange = msg.getUnderlyingScurityExchange();
        if ( tUnderlyingScurityExchange != null ) _builder.encodeBytes( FixDictionaryMD44.underlyingScurityExchange, tUnderlyingScurityExchange.getVal() );        // tag9803
        final ExchangeCode tPrimarySecurityExchange = msg.getPrimarySecurityExchange();
        if ( tPrimarySecurityExchange != null ) _builder.encodeBytes( FixDictionaryMD44.primarySecurityExchange, tPrimarySecurityExchange.getVal() );        // tag9804

        {
            ...
            _builder.encodeInt( FixDictionaryMD44.noSecurityAltID, counterSecurityAltID );
            int i=0;

            while ( tmpSecurityAltID != null ) { 
                ++i;
                _builder.encodeString( FixDictionaryMD44.securityAltID, tmpSecurityAltID.getSecurityAltID() );        // tag455
                final SecurityIDSource tSecurityAltIDSource = tmpSecurityAltID.getSecurityAltIDSource();
                if ( tSecurityAltIDSource != null ) _builder.encodeByte( FixDictionaryMD44.securityAltIDSource, tSecurityAltIDSource.getVal() );        // tag456
...

        {
            ...
            _builder.encodeInt( FixDictionaryMD44.noSDFeedTypes, counterSDFeedType );
            int i=0;

            while ( tmpSDFeedType != null ) { 
                ++i;
                _builder.encodeString( FixDictionaryMD44.feedType, tmpSDFeedType.getFeedType() );        // tag1022
                _builder.encodeInt( FixDictionaryMD44.marketDepth, tmpSDFeedType.getMarketDepth() );        // tag264
        ...
        // tag1141
        _builder.encodeInt( FixDictionaryMD44.maturityMonthYear, msg.getMaturityMonthYear() );        // tag200
        ...
        _builder.encodePrice( FixDictionaryMD44.contractMultiplier, msg.getContractMultiplier() );        // tag231
        ...
        _builder.encodeLong( FixDictionaryMD44.commonSecurityId, msg.getCommonSecurityId() );        // tag9805
        _builder.encodeLong( FixDictionaryMD44.parentCompanyId, msg.getParentCompanyId() );        // tag9806
        _builder.encodeString( FixDictionaryMD44.companyName, msg.getCompanyName() );        // tag9811
        _builder.encodeLong( FixDictionaryMD44.gicsCode, msg.getGicsCode() );        // tag9812
        _builder.encodeUTCTimestamp( FixDictionaryMD44.getOutDate, msg.getGetOutDate() );        // tag9813
        _builder.encodeUTCTimestamp( FixDictionaryMD44.deadTimestamp, msg.getDeadTimestamp() );        // tag9815
        _builder.encodeUTCTimestamp( FixDictionaryMD44.startTimestamp, msg.getStartTimestamp() );        // tag9817
        _builder.encodeUTCTimestamp( FixDictionaryMD44.endTimestamp, msg.getEndTimestamp() );        // tag9816
        final DataSrc tDataSrc = msg.getDataSrc();
        if ( tDataSrc != null ) _builder.encodeBytes( FixDictionaryMD44.dataSrc, tDataSrc.getVal() );        // tag9818
        final SecDefSpecialType tSecDefSpecialType = msg.getSecDefSpecialType();
        if ( tSecDefSpecialType != null ) _builder.encodeByte( FixDictionaryMD44.SecDefSpecialType, tSecDefSpecialType.getVal() );        // tag9819
        final CompanyStatusType tCompanyStatusType = msg.getCompanyStatusType();
        if ( tCompanyStatusType != null ) _builder.encodeBytes( FixDictionaryMD44.CompanyStatusType, tCompanyStatusType.getVal() );        // tag9820
        _builder.encodeEnvelope();
    }
    ...
    public final void encodeMDIncRefresh( final MDIncRefresh msg ) {
        ...
        _builder.encodeByte( 35, MSG_MDIncRefresh );
            ...
            _builder.encodeString( FixDictionaryMD44.TargetCompID, _targetCompId ); // tag56;
        _builder.encodeInt( FixDictionaryMD44.MsgSeqNum, msg.getMsgSeqNum() );        // tag34
            writeDateTime( FixDictionaryMD44.SendingTime, msg.getEventTimestamp() ); // tag52;

        {
            MDEntryImpl tmpMDEntry = (MDEntryImpl)msg.getMDEntries();
            int counterMDEntry = msg.getNoMDEntries();
            _builder.encodeInt( FixDictionaryMD44.noMDEntries, counterMDEntry );
            int i=0;

            while ( tmpMDEntry != null ) { 
                ++i;
                final MDUpdateAction tMdUpdateAction = tmpMDEntry.getMdUpdateAction();
                if ( tMdUpdateAction != null ) _builder.encodeByte( FixDictionaryMD44.mdUpdateAction, tMdUpdateAction.getVal() );        // tag279
                _builder.encodeInt( FixDictionaryMD44.mdPriceLevel, tmpMDEntry.getMdPriceLevel() );        // tag1023
                final MDEntryType tMdEntryType = tmpMDEntry.getMdEntryType();
                if ( tMdEntryType != null ) _builder.encodeByte( FixDictionaryMD44.mdEntryType, tMdEntryType.getVal() );        // tag269
                _builder.encodeInt( FixDictionaryMD44.mdEntryTime, tmpMDEntry.getMdEntryTime() );        // tag273
                final SecurityIDSource tSecurityIDSource = tmpMDEntry.getSecurityIDSource();
                if ( tSecurityIDSource != null ) _builder.encodeByte( FixDictionaryMD44.securityIDSource, tSecurityIDSource.getVal() );        // tag22
                _builder.encodeString( FixDictionaryMD44.securityID, tmpMDEntry.getSecurityID() );        // tag48
                _builder.encodeInt( FixDictionaryMD44.repeatSeq, tmpMDEntry.getRepeatSeq() );        // tag83
                _builder.encodePrice( FixDictionaryMD44.mdEntryPx, tmpMDEntry.getMdEntryPx() );        // tag270
                _builder.encodeInt( FixDictionaryMD44.mdEntrySize, tmpMDEntry.getMdEntrySize() );        // tag271
                _builder.encodeInt( FixDictionaryMD44.numberOfOrders, tmpMDEntry.getNumberOfOrders() );        // tag346
    ...
    public final void encodeMDSnapshotFullRefresh( final MDSnapshotFullRefresh msg ) {
...

        {
            ...
            _builder.encodeInt( FixDictionaryMD44.noMDEntries, counterMDSnapEntry );
            int i=0;
            ...
            while ( tmpMDSnapEntry != null ) { 
                ...
                if ( tMdEntryType != null ) _builder.encodeByte( FixDictionaryMD44.mdEntryType, tMdEntryType.getVal() );        // tag269
                _builder.encodePrice( FixDictionaryMD44.mdEntryPx, tmpMDSnapEntry.getMdEntryPx() );        // tag270
                _builder.encodeInt( FixDictionaryMD44.mdEntrySize, tmpMDSnapEntry.getMdEntrySize() );        // tag271
                _builder.encodeInt( FixDictionaryMD44.mdEntryTime, tmpMDSnapEntry.getMdEntryTime() );        // tag273
                final TickDirection tTickDirection = tmpMDSnapEntry.getTickDirection();
                if ( tTickDirection != null ) _builder.encodeByte( FixDictionaryMD44.tickDirection, tTickDirection.getVal() );        // tag274
                _builder.encodeInt( FixDictionaryMD44.tradeVolume, tmpMDSnapEntry.getTradeVolume() );        // tag1020
                _builder.encodeInt( FixDictionaryMD44.mdPriceLevel, tmpMDSnapEntry.getMdPriceLevel() );        // tag1023
    ...
    @Override public FixEncoder newInstance() {
        ...
        CMEMDEncoder e = new CMEMDEncoder( getComponentId(), _majorVersion, _minorVersion, buf, offset );
