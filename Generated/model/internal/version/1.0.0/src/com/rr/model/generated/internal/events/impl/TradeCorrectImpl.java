...
public final class TradeCorrectImpl implements TradeBase, TradeCorrectWrite, Copyable<TradeCorrect>, MarketTradeCorrectWrite, Reusable<TradeCorrectImpl> {

   // Attrs

    private transient          TradeCorrectImpl _next = null;
    private transient volatile Event        _nextMessage    = null;
    private transient          EventHandler _messageHandler = null;
    private final ReusableString _execRefID = new ReusableString( SizeType.EXECID_LENGTH.getSize() );
    private double _origQty = Constants.UNSET_DOUBLE;
   ...
   // Getters and Setters
    ...
    @Override public final double getOrigQty() { return _origQty; }
    @Override public final void setOrigQty( double val ) { _origQty = val; }
   ...
   // Reusable Contract
    ...
    public final void reset() {
        _execRefID.reset();
        _origQty = Constants.UNSET_DOUBLE;
    ...
    public final ReusableType getReusableType() {
        return ModelReusableTypes.TradeCorrect;
    }

    @Override
    public final TradeCorrectImpl getNext() {
        return _next;
    }

    @Override
    public final void setNext( TradeCorrectImpl nxt ) {
        _next = nxt;
    }
   ...
   // Helper methods
    ...
    public final void dump( final ReusableString out ) {
        out.append( "TradeCorrectImpl" ).append( ' ' );
        if ( getExecRefID().length() > 0 )             out.append( ", execRefID=" ).append( getExecRefID() );
        if ( Utils.hasVal( getOrigQty() ) ) out.append( ", origQty=" ).append( getOrigQty() );
        if ( Utils.hasVal( getOrigPx() ) ) out.append( ", origPx=" ).append( getOrigPx() );
    ...
    @Override public final void snapTo( TradeCorrect dest ) {
        ((TradeCorrectImpl)dest).deepCopyFrom( this );
    }

    /** DEEP copy all members ... INCLUDING subEvents : WARNING CREATES NEW OBJECTS SO MONITOR FOR GC */
    @Override public final void deepCopyFrom( TradeCorrect src ) {
        getExecRefIDForUpdate().copy( src.getExecRefID() );
        setOrigQty( src.getOrigQty() );
        setOrigPx( src.getOrigPx() );
        setLastQty( src.getLastQty() );
        setLastPx( src.getLastPx() );
        setLiquidityInd( src.getLiquidityInd() );
        setMultiLegReportingType( src.getMultiLegReportingType() );
    ...
    @Override public final void shallowCopyFrom( TradeCorrect src ) {
        getExecRefIDForUpdate().copy( src.getExecRefID() );
        setOrigQty( src.getOrigQty() );
        setOrigPx( src.getOrigPx() );
        setLastQty( src.getLastQty() );
    ...
    /** shallow copy all primitive members ... EXCLUDING subEvents */
    @Override public final void shallowMergeFrom( TradeCorrect src ) {
        if ( src.getExecRefID().length() > 0 ) getExecRefIDForUpdate().copy( src.getExecRefID() );
        if ( Utils.hasVal( src.getOrigQty() ) ) setOrigQty( src.getOrigQty() );
        if ( Utils.hasVal( src.getOrigPx() ) ) setOrigPx( src.getOrigPx() );
