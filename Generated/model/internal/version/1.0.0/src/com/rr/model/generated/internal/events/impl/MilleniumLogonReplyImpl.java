...
public final class MilleniumLogonReplyImpl implements BaseMillenium, MilleniumLogonReplyWrite, Copyable<MilleniumLogonReply>, Reusable<MilleniumLogonReplyImpl> {

   // Attrs

    private transient          MilleniumLogonReplyImpl _next = null;
   ...
   // Helper methods
    ...
    public final void dump( final ReusableString out ) {
        ...
        if ( Constants.UNSET_INT != getRejectCode() && 0 != getRejectCode() )             out.append( ", rejectCode=" ).append( getRejectCode() );
        if ( getPwdExpiryDayCount().length() > 0 )             out.append( ", pwdExpiryDayCount=" ).append( getPwdExpiryDayCount() );
    ...
    @Override public final void snapTo( MilleniumLogonReply dest ) {
        ((MilleniumLogonReplyImpl)dest).deepCopyFrom( this );
    }

    /** DEEP copy all members ... INCLUDING subEvents : WARNING CREATES NEW OBJECTS SO MONITOR FOR GC */
    @Override public final void deepCopyFrom( MilleniumLogonReply src ) {
        setRejectCode( src.getRejectCode() );
        getPwdExpiryDayCountForUpdate().copy( src.getPwdExpiryDayCount() );
    ...
    @Override public final void shallowCopyFrom( MilleniumLogonReply src ) {
        setRejectCode( src.getRejectCode() );
        getPwdExpiryDayCountForUpdate().copy( src.getPwdExpiryDayCount() );
        setMsgSeqNum( src.getMsgSeqNum() );
        setPossDupFlag( src.getPossDupFlag() );
    ...
    /** shallow copy all primitive members ... EXCLUDING subEvents */
    @Override public final void shallowMergeFrom( MilleniumLogonReply src ) {
        if ( Constants.UNSET_INT != src.getRejectCode() ) setRejectCode( src.getRejectCode() );
        if ( src.getPwdExpiryDayCount().length() > 0 ) getPwdExpiryDayCountForUpdate().copy( src.getPwdExpiryDayCount() );
